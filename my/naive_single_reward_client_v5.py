#!/usr/bin/env python3
"""
Naive reward client V5.

版本定位（快速区分）:
- 对接 `NaiveSingleRewardServerV5`（`/compute_reward_v5`）。
- 信号集合 = 5 个 render 一层信号 + 3 个 VLM 静态视觉信号（共 8 维）。
- 支持 `visual_judge_mode`: `joint`（默认）或 `separate`。
...
DEFAULT_SERVER_URL = "http://127.0.0.1:48004/compute_reward_v5"
